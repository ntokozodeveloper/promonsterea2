...
async def fetch_deriv_data(api, symbol, interval, start_date, end_date):
    try:
        ...
        deriv_interval = interval_mapping.get(
            interval, '1d', '1h', '4h', '30m', '15m', '5m', '1m')  # Default to theses vaules if other time frames are not found.
...
async def trade():
    ...
    intervals = ['1mo', '1wk', '1d', '1h', '2h', '3h', '30m', '15m']
    data_frames = {'1mo', '1wk', '1d', '1h', '2h', '3h', '30m', '15m'}
...
@app.route('/')
def serve_index():
    return send_from_directory(os.path.join(app.root_path, 'backend'), 'index.html')
