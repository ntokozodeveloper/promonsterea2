...
const newLocal = new WebSocket(
  "wss://ws.derivws.com/websockets/v3?app_id=61999",
);
...
app.use(express.static("web-based-ea-frontend/index.html")); // Replace 'client/build' with your React app's build folder
// Serve the HTML form
app.get("/", (req, res) => {
...
app.post("/trade", async (req, res) => {
  const { serialKey, symbol, amount, contract_type, app_id, api_token } =
    req.body;
  ...
  try {
    ...
    console.error(
      "Error sending request to Flask server, This is due to an Invalid Deriv APP ID or Token:",
      error.message,
    );
...
app.use((req, res, next) => {
  res.status(404).send("Not Found, Undefined Route.");
});
...
// Get Trade Route and style the Path
app.get('/trade', async (req, res) => {
  try {
      const response = await axios.get('http://localhost:5000/getTradeData'); // URL of your Python API
